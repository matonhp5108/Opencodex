...
export class TerminalManager {
  ...
  start(name: string, cwd: string, initialCommand?: string): string {
    ...
    if (existing && existing.exitCode === undefined)
      throw new Error(`Terminal '${sessionName}' is already running.`);
    ...
    if (initialCommand?.trim())
      child.stdin.write(`${initialCommand.trim()}${shell.lineEnding}`);
  ...
  read(
    ...
    const safeCursor = Math.max(
      session.baseOffset,
      Math.min(cursor, absoluteEnd),
    );
    return {
      output:
        session.output.slice(safeCursor - session.baseOffset) ||
        "(no new output)",
      cursor: absoluteEnd,
  ...
  private running(name: string): TerminalSession {
    ...
    if (session.exitCode !== undefined)
      throw new Error(
        `Terminal '${session.name}' has exited with code ${session.exitCode ?? "unknown"}.`,
      );
    return session;
  }
}
