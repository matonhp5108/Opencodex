...
function notificationsEnabled(): boolean {
  return vscode.workspace.getConfiguration('opencodex').get<boolean>(SETTING_ENABLED, true);
}
...
function notifyMacOS(context: vscode.ExtensionContext, title: string, message: string): void {
  const binary = context.asAbsolutePath(path.join('native', 'OpencodexNotifier.app', 'Contents', 'MacOS', 'OpencodexNotifier'));
...
function notifyWindows(context: vscode.ExtensionContext, title: string, message: string): void {
  ...
  const icon = context.asAbsolutePath(path.join('media', 'opencodex-notification.png'));
