...
import {
  ...
  finishMcpOAuth,
  testMcpConnection,
  connectToMcpConnections,
  checkMcpConnectionStatuses,
} from "./mcp";
...
function isProjectMeta(
  entry: unknown,
): entry is ProjectMetaEntry & { id: string; name: string; path: string } {
...
export class AgentViewProvider implements vscode.WebviewViewProvider {
  ...
  private pendingNotifies = new Map<
    number,
    (choice: "ok" | "secondary" | "cancel") => void
  >();
  ...
  async openMemory(): Promise<void> {
    ...
    if (!root) {
      void vscode.window.showInformationMessage(
        "Open a folder or workspace first.",
      );
      return;
    }
  ...
  showAddMcp(): void {
    this.view?.show?.(true);
    this.post({ type: "showAddMcp" });
  }

  async handleUri(uri: vscode.Uri): Promise<void> {
    if (uri.path !== "/mcp/oauth/callback") return;
    try {
      ...
      this.post({
        ...
        message: `${name} is connected and ready to test.`,
        kind: "info",
      });
      void vscode.window.showInformationMessage(
        `Opencodex: ${name} OAuth authorization completed.`,
      );
      const connections = await loadMcpConnections(this.context);
      this.post({ type: "mcpConnections", connections });
      const statuses = await checkMcpConnectionStatuses(this.context);
      this.post({ type: "mcpConnectionStatuses", statuses });
    } catch (error) {
      ...
      void vscode.window.showErrorMessage(`Opencodex MCP OAuth: ${message}`);
    }
  }
  ...
  private async ensureGlobalSkillsOnce(): Promise<void> {
    try {
      if (
        this.context.globalState.get<boolean>("opencodex.skillsMigrated", false)
      )
        return;
      const root = this.workspaceRoot();
      if (!root) {
      ...
      const skillFolders = entries.filter(
        ([, type]) => (type & vscode.FileType.Directory) !== 0,
      );
    ...
    for (const [name, type] of entries) {
      ...
      if ((type & vscode.FileType.Directory) !== 0)
        ...
        await vscode.workspace.fs.writeFile(
          to,
          await vscode.workspace.fs.readFile(from),
        );
    }
  }
  ...
  private async sendMarketplaceInstalled(): Promise<void> {
    ...
    const sources =
      this.context.globalState.get<Record<string, string>>(
        "opencodex.skillSources",
        {},
      ) ?? {};
    const skills = (await listInstalledSkills(this.skillsRoot())).map(
      (skill) => ({ ...skill, source: sources[skill.folder] ?? "" }),
    );
  ...
  private async showSettings(initialSetup = false): Promise<void> {
    ...
    this.post({
      ...
      providers: listProviders(this.customProviders).map((provider) => ({
        ...
        name: provider.name,
        needsApiKey: provider.needsApiKey,
        acceptsApiKey: provider.acceptsApiKey ?? provider.needsApiKey,
      ...
      apiKeys: Object.fromEntries(
        listProviders(this.customProviders).map((provider) => [
          ...
          Boolean(
            ...
            (provider.apiKeyEnvVar ? process.env[provider.apiKeyEnvVar] : ""),
          ),
        ]),
      ),
  ...
  private async maybeShowFirstLaunchSettings(): Promise<void> {
    if (this.context.globalState.get<boolean>("opencodex.setupComplete", false))
      return;
  ...
  private loadProjects(): void {
    const stored = this.context.globalState.get<unknown[]>(
      "opencodex.projectIndex",
      [],
    );
    this.projects = (Array.isArray(stored) ? stored : [])
      .filter(isProjectMeta)
      .map((entry) => {
        const data = this.context.globalState.get<
          | { conversations?: unknown; activeConversationId?: unknown }
          | undefined
        >(`opencodex.project.${entry.id}`, undefined);
        ...
        return {
          ...
          conversations: conversations.map((conversation) => {
            const baseTimestamp = Number.isFinite(conversation.createdAt)
              ? conversation.createdAt
              : Date.now();
            return {
              ...conversation,
              items: conversation.items.map((item, index) =>
                normalizeTranscriptItem(item, baseTimestamp + index),
              ),
            };
          }),
        };
    ...
    this.projects = this.projects.filter((project) =>
      seenIds.has(project.id) ? false : (seenIds.add(project.id), true),
    );
    const root = this.workspaceRoot()?.fsPath;
    if (root) {
      let project = this.projects.find((item) => item.path === root);
  ...
  private migrateLegacyWorkspaceState(project: Project): void {
    const legacy = this.context.workspaceState.get<Conversation[]>(
      "opencodex.conversations",
      [],
    );
    const legacyTranscript = this.context.workspaceState.get<TranscriptItem[]>(
      "opencodex.transcript",
      [],
    );
    if (
      (!legacy.length && !legacyTranscript.length) ||
    ...
    const conversations = legacy.length
      ...
      : [
          this.createConversation(
            legacyTranscript.map((item, index) =>
              normalizeTranscriptItem(item, Date.now() + index),
            ),
          ),
        ];
    project.conversations = conversations;
    const saved = this.context.workspaceState.get<string>(
      "opencodex.activeConversationId",
      "",
    );
    project.activeConversationId = conversations.some(
      (item) => item.id === saved && !item.archived,
    )
      ? saved
      : (conversations.find((item) => !item.archived)?.id ?? "");
  ...
  private createConversation(items: TranscriptItem[] = []): Conversation {
    ...
    return {
      ...
      title: first ? conversationTitle(first) : "New conversation",
      items,
      archived: false,
      createdAt: now,
      updatedAt: now,
  ...
  private activeConversation(): Conversation | undefined {
    ...
    let conversation = project.conversations.find(
      (item) => item.id === project.activeConversationId,
    );
    if (!conversation) {
  ...
  private newConversation(): void {
    const project = this.activeProject();
    if (!project) return;
    const empty = project.conversations.find(
      (item) => !item.archived && item.items.length === 0,
    );
  ...
  private persistProjects(): Promise<void> {
    ...
    this.persistChain = this.persistChain.then(async () => {
      await this.context.globalState.update(
        "opencodex.projectIndex",
        projects.map(({ id, name, path, createdAt, updatedAt }) => ({
          id,
          name,
          path,
          createdAt,
  ...
  private syncConversations(includeActive = true): void {
    ...
    this.post({
      type: "project",
      name: root ? path.basename(root.fsPath) || root.fsPath : "No folder open",
      path: root ? root.fsPath : "",
    });
    const project = this.activeProject();
    if (!project) return;
    this.post({
      ...
      conversations: project.conversations.map(
        ({ id, title, archived, updatedAt, items }) => ({
          id,
          title,
          archived,
          updatedAt,
          ...
          queued:
            this.queue.find((entry) => entry.conversationId === id)?.text ??
            null,
        }),
      ),
  ...
  private confirmDeleteConversations(): boolean {
    return (
      this.context.globalState.get<boolean>("opencodex.confirmDelete", true) !==
      false
    );
  }
  ...
  private async onMessage(message: WebMessage): Promise<void> {
    ...
    if (message.type === "nativeNotify") {
      const text = message.detail
        ? `${message.message}\n\n${message.detail}`
        : message.message;
      if (message.kind === "error") void vscode.window.showErrorMessage(text);
    ...
    if (message.type === "openConversation") {
      ...
      if (
        project &&
        project.conversations.some((item) => item.id === message.id)
      ) {
    ...
    if (message.type === "archiveConversation") {
      ...
      this.queue = this.queue.filter(
        (entry) => entry.conversationId !== message.id,
      );
      ...
      if (
        ...
        const next = project.conversations.find(
    ...
    if (message.type === "deleteConversation") {
      ...
      if (project.activeConversationId === message.id) {
        const next =
          project.conversations.find((item) => !item.archived) ??
          this.createConversation();
        if (!project.conversations.includes(next))
      ...
      systemNotify(this.context, {
        subtitle: "Conversation deleted",
        message: "Conversation '" + conversation.title + "' was deleted.",
      });
      return;
    }
    if (message.type === "restoreCheckpoint") {
      ...
      if (
        !root ||
        !project ||
        project.path !== root.fsPath ||
        !isGitTrackedWorkspace(root.fsPath)
      ) {
        void vscode.window.showInformationMessage(
          "Restore is available only for the Git-tracked project that matches the current folder.",
        );
        return;
      }
      const conversation = project.conversations.find(
      ...
      const targetIndex =
        conversation?.items.findIndex(
          (item) => item.id === message.itemId && item.role === "assistant",
        ) ?? -1;
      if (!conversation || targetIndex < 0) return;
      const target = conversation.items[targetIndex];
      if (!target?.gitTree) {
        void vscode.window.showInformationMessage(
          "This message does not have a Git restore point. Restore points are created for newer Opencodex responses.",
        );
        return;
      }
      ...
      const choice = await this.prompt(
        "Restore to before this response?",
        detail,
        { ok: "Restore", cancel: "Cancel", danger: true },
      );
      if (choice !== "ok") return;
      try {
        ...
        void vscode.window.showErrorMessage(
          `Git restore failed: ${errorMessage(error)}`,
        );
        return;
      }
      ...
      this.syncConversations();
      void vscode.window.showInformationMessage(
        "Git workspace and conversation restored.",
      );
      return;
    }
    if (message.type === "setKey" || message.type === "requestSettings")
      return this.showSettings();
    if (message.type === "requestUsage") return this.openUsage();
    if (message.type === "requestMarketplace") return this.openMarketplace();
    if (message.type === "requestMarketplaceInstalled")
      return void this.sendMarketplaceInstalled();
    if (message.type === "requestAddMcp") return this.showAddMcp();
    if (message.type === "showAddMcp") {
      this.post({ type: "showAddMcp", connection: message.connection });
      return;
    }
    if (message.type === "saveMcpConnection") {
      try {
        await saveMcpConnection(message.connection, this.context);
        this.post({
          type: "mcpConnectionSaved",
        ...
        this.post({
          type: "toast",
          id: ++this.notifySeq,
          title: "Error saving MCP connection",
    ...
    if (message.type === "testMcpConnection") {
      try {
        ...
        this.post({
          ...
          error: result.error,
          tools: result.tools,
          authorizationStarted: result.authorizationStarted,
          discoveredOAuth: result.discoveredOAuth,
        });
      } catch (error) {
        this.post({
          type: "mcpConnectionTestResult",
    ...
    if (message.type === "deleteMcpConnection") {
      try {
        await deleteMcpConnection(message.name, this.context);
        this.post({ type: "mcpConnectionDeleted", name: message.name });
      } catch (error) {
        this.post({
          type: "toast",
          id: ++this.notifySeq,
          title: "Error deleting MCP connection",
          message: errorMessage(error),
          kind: "attention",
        });
      }
    ...
    if (message.type === "fetchMcpConnections") {
      try {
        const connections = await loadMcpConnections(this.context);
        this.post({ type: "mcpConnections", connections });
      } catch (error) {
        this.post({
          type: "toast",
          id: ++this.notifySeq,
          title: "Error loading MCP connections",
          message: errorMessage(error),
          kind: "attention",
        });
      }
    ...
    if (message.type === "checkMcpConnections") {
      try {
        const statuses = await checkMcpConnectionStatuses(this.context);
        this.post({ type: "mcpConnectionStatuses", statuses });
      } catch (error) {
        this.post({
          type: "toast",
          id: ++this.notifySeq,
          title: "Error checking MCP connections",
          message: errorMessage(error),
          kind: "attention",
        });
      }
    ...
    if (message.type === "marketplaceTop") {
      try {
        const { skills, total } = await searchSkills("skill", {
          limit: 20,
          sortBy: message.sortBy ?? "stars",
        });
    ...
    if (message.type === "marketplaceSearch") {
      try {
        const { skills, total } = await searchSkills(message.query, {
          limit: Math.max(1, Math.min(50, message.limit || 10)),
          sortBy: message.sortBy,
        });
        this.post({
          type: "marketplaceResults",
          query: message.query,
          total,
          skills,
        });
    ...
    if (message.type === "marketplaceListRepo") {
      try {
        const reference = resolveInstallPath(
          message.source,
          "",
          message.branch ?? "main",
        );
    ...
    if (message.type === "marketplacePreview") {
      try {
        ...
        this.post({
          ...
          title: reference.owner + "/" + reference.repo + " / " + folderPath,
      ...
      try {
        ...
        if (!folderPath) {
          ...
          const match = requested
            ? skills.find(
                (candidate) =>
                  candidate.name === sanitizeSkillName(requested) ||
        ...
        await this.approve(
          ...
          "Source: " +
            ...
            "\n\nThe skill will be installed into your global skills folder as '" +
            installName +
            "' and is available in every workspace.",
        );
    ...
    if (message.type === "marketplaceUninstall") {
      ...
      try {
        ...
        const skill = installed.find(
          (item) =>
            item.folder === safeName ||
            sanitizeSkillName(item.name) === safeName,
        );
        if (!skill) {
          this.post({
            type: "marketplaceResult",
            ok: false,
            text: "Skill '" + (message.folder || "") + "' not found.",
          });
          return;
        }
        const choice = await this.prompt(
          ...
          "This removes '" +
            skill.name +
            "' from your global skills folder. It will no longer be offered to the agent in any workspace.",
          { ok: "Uninstall", cancel: "Cancel", danger: true },
        );
        if (choice !== "ok") return;
    ...
    if (message.type === "saveSettings") {
      try {
        ...
        if (searxngUrl && !/^https?:\/\//i.test(searxngUrl))
          throw new Error("SearXNG URL must start with http:// or https://.");
        ...
        await config.update(
          "extraFreeModels",
          message.extraFreeModels ?? "",
          vscode.ConfigurationTarget.Global,
        );
        await this.context.globalState.update(
          "opencodex.approvalMode",
          normalizeApprovalMode(message.approvalMode),
        );
        await this.context.globalState.update(
        ...
        await this.context.globalState.update(
          "opencodex.systemPrompt",
          message.systemPrompt ?? "",
        );
        await this.context.globalState.update(
          "opencodex.onlyDefaultModels",
          Boolean(message.onlyDefaultModels),
        );
        await this.context.globalState.update(
          "opencodex.confirmDelete",
        ...
        if (provider.isLocal) {
          ...
          await this.context.globalState.update(
            `opencodex.baseUrl.${providerId}`,
            baseUrl || undefined,
          );
        }
        if (
          message.apiKey.trim() &&
          (provider.acceptsApiKey ?? provider.needsApiKey)
        ) {
          await this.context.secrets.store(
            `opencodex.apiKey.${providerId}`,
            message.apiKey.trim(),
          );
    ...
    if (message.type === "chooseContext") {
      ...
      for (const uri of chosen ?? []) {
        ...
        attachments.push({
          kind:
            (stat.type & vscode.FileType.Directory) !== 0 ? "folder" : "file",
    ...
    if (message.type === "revealSkill") {
      ...
      try {
        ...
        void vscode.window.showInformationMessage(
          "Could not open the skill in your file explorer.",
        );
      }
      return;
    }
    if (message.type === "send" && message.text.trim()) {
      ...
      let conversationId =
        message.conversationId || project.activeConversationId;
      if (
      ...
      const root = this.workspaceRoot();
      const promptContext = root
        ? await this.composerContextBlock(root, message.context)
        : "";
      if (
        this.runs.has(conversationId) ||
        this.runs.size >= MAX_CONCURRENT_RUNS
      ) {
        this.enqueue(
          message.text.trim(),
          conversationId,
    ...
    if (message.type === "retryMessage") {
      ...
      if (!project || !conversation || this.runs.has(message.conversationId))
        return;
  ...
  private async run(
    userText: string,
    conversationId: string,
    resume?: { work?: WorkItem[]; errorText?: string },
    carryTree?: string,
    composerContext?: ComposerContext,
    preparedPromptContext?: string,
  ): Promise<void> {
    const root = this.workspaceRoot();
    if (!root) {
      this.post({ type: "error", text: "Open a folder or workspace first." });
    ...
    const promptContext = resume
      ? ""
      : (preparedPromptContext ??
        (await this.composerContextBlock(root, composerContext)));
    ...
    if (resume) {
      ...
      if (conversation.items.length === 1)
        conversation.title = conversationTitle(userText);
    ...
    const describePlan = (): string => {
      ...
      if (!current)
        return "No plan yet: call the plan tool with title and steps to create one.";
      if (!current.steps.length) return `Current plan: ${current.title}`;
      const lines = current.steps.map((step, index) => {
        const status = current.done.has(index)
          ? "done"
    ...
    if (resume) {
      const priorPlan = [...work]
        .reverse()
        .find((item) => item.kind === "plan");
      if (priorPlan) {
        const steps = priorPlan.steps ?? [];
        const done = new Set<number>(
          (priorPlan.doneSteps ?? []).filter((index) =>
            Number.isInteger(index),
          ),
        );
        let active =
          Number.isInteger(priorPlan.activeStep) &&
          (priorPlan.activeStep ?? -1) >= 0
            ? (priorPlan.activeStep as number)
            : 0;
        if (active >= steps.length) active = -1;
        planState = {
          title: priorPlan.title ?? "Plan",
          steps,
          active,
          done,
          manual: Boolean(priorPlan.manual),
          interrupted: false,
        };
        planItem = priorPlan;
    ...
    try {
      ...
      if (!model) {
        ...
        this.post({
          type: "error",
          text: "Choose a model from the selector below the message box.",
        });
        return;
      }
      ...
      const provider = createOpenAICompatible({
        ...
        ...(providerConfig.id === "openrouter"
          ? {
              headers: {
                "HTTP-Referer": "https://github.com/matonhp5108/Opencodex",
                "X-Title": "Opencodex",
              },
            }
          : {}),
      ...
      if (Array.isArray(savedConnections) && savedConnections.length > 0) {
        mcpConnection = await connectToMcpConnections(
          savedConnections,
          root.fsPath,
      ...
      const instructions = [
        ...
        projectMemory
          ? `Durable project memory from ${MEMORY_RELATIVE_PATH}:\n${projectMemory}`
          : "",
        ...
        mcpConnection.instructions.length
          ? `Connected MCP server instructions:\n${mcpConnection.instructions.join("\n")}`
          : "",
        mcpConnection.errors.length
          ? `Some configured MCP servers could not connect:\n- ${mcpConnection.errors.join("\n- ")}`
          : "",
        skillBlock,
      ]
        .filter(Boolean)
      ...
      const memoryAccess = {
        ...
        write: async (content: string, reason?: string) => {
          ...
          await this.reviewEdit(
            ...
            before,
            content,
            reason ?? "Store durable project context for future conversations.",
          );
      ...
      const delegate = async (
        ...
        const subagentTools = buildTools({
          ...
          config: () => this.config(),
          approve: (kind, title, detail, destructive) =>
            this.approve(kind, title, detail, destructive),
          reviewEdit: (filePath, before, after, reason, destructive) =>
            this.reviewEdit(filePath, before, after, reason, destructive),
          post: (message) => this.post(message),
          resolvePath: (filePath) => this.resolveWorkspacePath(filePath),
          describePlan: () =>
            "Subagents do not publish a parent plan. Work directly on the assigned task.",
        ...
        const subagent = new ToolLoopAgent({
          ...
          tools: {
            ...subagentTools,
            ...(role === "worker" ? mcpConnection?.tools : {}),
          },
          stopWhen: isStepCount(
            maxSteps === 0 ? 12 : Math.max(2, Math.min(12, maxSteps)),
          ),
        });
        ...
        return (
          result.text.trim() || "(Subagent completed without a text response.)"
        );
      };
      ...
      if (resume) {
        ...
        const planLines = plan?.steps
          ? plan.steps
              .map(
                (step, index) =>
                  `- ${plan.done.has(index) ? "[done]" : index === plan.active ? "[current]" : "[pending]"} ${step}`,
              )
              .join("\n")
          : "";
        const inProgressLines = work
          .filter((item) => item.kind === "task" && item.done === false)
          .map((item) => `- ${item.text.replace(/\s+/g, " ")}`)
          .join("\n");
        const doneLines = work
          .filter((item) => item.kind === "task" && item.done !== false)
          .slice(-10)
          .map((item) => `- ${item.text.replace(/\s+/g, " ")}`)
          .join("\n");
        resumeContext = [
          planLines ? `Task plan:\n${planLines}` : "",
          inProgressLines
            ? `Current task (continue from here):\n${inProgressLines}`
            : "",
          doneLines
            ? `Work already completed (do not redo):\n${doneLines}`
            : "",
          resume?.errorText
            ? `The last attempt ended with:\n${resume.errorText}`
            : "",
        ]
          .filter(Boolean)
          .join("\n\n");
        ...
        const recent = conversation.items
          .slice(-10, -1)
      ...
      do {
        ...
        const result = await agent.stream({
          ...
          onToolExecutionStart: ({ toolCall }) => {
            if (toolCall.toolName === "plan") {
              ...
              for (const index of input?.doneSteps ?? []) {
                if (Number.isInteger(index) && (index as number) >= 0)
                  passedDone.add(index as number);
              }
              if (parsedSteps.length) {
                ...
                for (const index of passedDone)
                  if (index < parsedSteps.length) done.add(index);
                for (const index of planState?.done ?? [])
                  if (index < parsedSteps.length) done.add(index);
                let active = Number.isInteger(input?.activeStep)
                  ? (input.activeStep as number)
                  : planState && planState.active >= 0
                    ? planState.active
                    : 0;
                if (active >= parsedSteps.length)
                  active = parsedSteps.length - 1;
                planState = {
                  title,
                  steps: parsedSteps,
                ...
                if (typeof input?.title === "string" && input.title.trim())
                  planState.title = input.title.trim();
                if (
                  Number.isInteger(input?.activeStep) &&
                  (input.activeStep as number) >= 0
                ) {
                  planState.active = Math.min(
                    planState.steps.length - 1,
                    input.activeStep as number,
                  );
                }
                planState.manual = true;
              ...
              if (planState) {
                planItem = {
                  kind: "plan",
                  text: planState.title,
                  title: planState.title,
                  steps: planState.steps,
                  doneSteps: [],
                  activeStep: planState.active,
                  interrupted: false,
                };
                let lastPlanIndex = -1;
                for (let index = work.length - 1; index >= 0; index--) {
                  if (work[index]?.kind === "plan") {
                    lastPlanIndex = index;
                    break;
                  }
                }
                if (lastPlanIndex >= 0) work[lastPlanIndex] = planItem;
                else work.push(planItem);
                postPlan();
              }
              return;
            }
            const taskEntry: WorkItem = {
              kind: "task",
              text: toolTask(toolCall.toolName, toolCall.input),
              done: false,
            };
            ...
            this.post({
              ...
              conversationId,
              running: true,
              label: humanToolName(toolCall.toolName),
            });
          },
          onToolExecutionEnd: ({ toolCall }) => {
            ...
            this.post({
              ...
              phase: "end",
              id: toolCall.toolCallId,
              name: toolTask(toolCall.toolName, toolCall.input),
            });
          },
        });
        ...
        for await (const part of result.stream) {
          if (part.type === "text-delta") {
            ...
            if (reasoningBuffer.length >= MAX_PERSISTED_REASONING)
        ...
        if (finishReason === "error")
          throw new Error(
            "The model stopped because the provider reported a generation error.",
          );
        if (finishReason === "content-filter")
          throw new Error(
            "The model stopped because the provider blocked the response.",
          );
      ...
      if (reasoningBuffer.trim())
        work.push({
          kind: "reasoning",
          text: reasoningBuffer + (reasoningTruncated ? "\n…(truncated)" : ""),
        });
      const keptWork = work.slice(-80);
      const workSeconds = workStartedAt
        ? Math.max(1, Math.round((Date.now() - workStartedAt) / 1000))
        : 0;
      ...
      systemNotify(this.context, {
        ...
        message: notificationSummary(answer) || conversation.title,
        kind: "info",
      });
    } catch (error) {
      if (run.controller.signal.aborted) {
        if (planState) {
          planState.interrupted = true;
          postPlan();
        }
        if (run.steering) {
        ...
        if (reasoningBuffer.trim())
          work.push({
            kind: "reasoning",
            text:
              reasoningBuffer + (reasoningTruncated ? "\n…(truncated)" : ""),
          });
        const errorItem = createTranscriptItem(
          "assistant",
          message,
          "error",
          runGitTree,
        ...
        systemNotify(this.context, {
          ...
          message: notificationSummary(message) || conversation.title,
          kind: "attention",
        });
      }
    } finally {
  ...
  private async reviewEdit(
    ...
    const safeBase = (path.basename(filePath) || "change.txt").replace(
      /[^a-zA-Z0-9._-]/g,
      "_",
    );
    ...
    try {
      ...
      systemNotify(this.context, {
        subtitle: "Edit review needed",
        message: filePath,
        kind: "attention",
      });
      const choice = await this.prompt(
        destructive
          ? `Review deletion of ${filePath}`
          : `Review proposed edit to ${filePath}`,
        `${reason}\n\nThe proposed diff is open in the editor. Apply it?`,
        {
          ok: destructive ? "Delete" : "Apply",
          cancel: "Reject",
          danger: destructive,
        },
      );
    ...
    return {
      ...
      approvalMode: normalizeApprovalMode(
        this.context.globalState.get<string>(
          "opencodex.approvalMode",
          config.get<string>("approvalMode", "ask"),
        ),
      ),
      searxngUrl: this.context.globalState.get<string>(
        "opencodex.searxngUrl",
        config.get<string>("searxngUrl", ""),
      ),
      systemPrompt: this.context.globalState.get<string>(
        "opencodex.systemPrompt",
        config.get<string>("systemPrompt", ""),
      ),
      mcpServers: this.context.globalState.get<string>(
        "opencodex.mcpServers",
        config.get<string>("mcpServers", "{}"),
      ),
      extraFreeModels: (config.get<string>("extraFreeModels", "") ?? "")
        .split(",")
        .map((item) => item.trim())
        .filter(Boolean),
      onlyDefaultModels: this.context.globalState.get<boolean>(
        "opencodex.onlyDefaultModels",
        true,
      ),
    };
  ...
  private async refreshModels(): Promise<void> {
      ...
      targets.map(async (provider) => {
        try {
          const models = await fetchProviderModels(
            ...
            config.extraFreeModels,
            this.providerBaseUrl(provider),
          );
          groups.push({
          ...
          groups.push({
            ...
            configured: this.providerConfigured(provider),
            models: [],
            error: friendlyError(error, provider),
          });
        }
      }),
    );
    groups.sort((a, b) =>
      a.providerId === config.provider
        ? -1
        : b.providerId === config.provider
          ? 1
          : a.providerName.localeCompare(b.providerName),
    );
    const configured = config.model;
    const selected =
      configured && groups.some((group) => group.models.includes(configured))
        ? configured
        : "";
    ...
    if (!anyModels) {
      ...
      this.post({
        type: "modelsError",
        text: details || "No free models found for any configured provider.",
      });
      return;
    }
    this.post({
  ...
  private sendEditorContext(): void {
    ...
    const inside = Boolean(
      root && editor && pathInside(root.fsPath, editorPath),
    );
    const selection =
      inside && editor && !editor.selection.isEmpty
        ? editor.selection
        : undefined;
    this.post({
      type: "editorContext",
  ...
  private async composerContextBlock(
    root: vscode.Uri,
    context?: ComposerContext,
  ): Promise<string> {
    ...
    const requestedFile = context?.activeFile
      ?.replace(/\\/g, "/")
      .replace(/^\.\//, "");
    const editorRelative = editorInside
      ? path.relative(root.fsPath, editorPath).replace(/\\/g, "/")
      : "";
    const editorMatchesRequest = Boolean(
      editor &&
      editorInside &&
      (!requestedFile || requestedFile === editorRelative),
    );
    if (
      editor &&
      editorMatchesRequest &&
      context?.includeSelection !== false &&
      ...
      const selected = editor.document
        ...
        .slice(0, 12_000);
      sections.push(
        `Selected code from ${relative}:${editor.selection.start.line + 1}-${editor.selection.end.line + 1}:\n\`\`\`\n${selected}\n\`\`\``,
      );
    }
    if (
      context?.includeActiveFile !== false &&
      (requestedFile || editorRelative)
    ) {
      const relative = requestedFile || editorRelative;
      if (!isSecret(relative)) {
        const content =
          editor && editorMatchesRequest
        ...
        sections.push(
          `Active file ${relative} (included via the active-file context control):\n\`\`\`\n${content.slice(0, 30_000)}\n\`\`\``,
        );
        seen.add(relative);
      }
    }
    for (const attachment of (context?.attachments ?? []).slice(0, 16)) {
      ...
      if (
        ...
        sections.push(
          `Attached folder: ${relative}. Inspect only the relevant files inside it.`,
        );
        ...
        sections.push(
          `Attached file: ${relative} (${stat.size} bytes; use read/search tools selectively).`,
        );
      }
      seen.add(relative);
    }
