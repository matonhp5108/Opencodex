...
export async function readProjectMemory(root: vscode.Uri): Promise<string> {
  try {
    const content = new TextDecoder().decode(await vscode.workspace.fs.readFile(memoryUri(root)));
...
export async function writeProjectMemory(root: vscode.Uri, content: string): Promise<void> {
  const normalized = content.trim().slice(0, MAX_MEMORY_CHARS);
  await vscode.workspace.fs.createDirectory(vscode.Uri.joinPath(root, '.opencodex'));
  await vscode.workspace.fs.writeFile(memoryUri(root), new TextEncoder().encode(`${normalized}\n`));
}
