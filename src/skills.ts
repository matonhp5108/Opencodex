...
type TreeEntry = { path: string; type: 'blob' | 'tree' | string; size?: number };
...
function parsedGithubReference(source: string): { owner: string; repo: string; branch: string; path: string } | undefined {
...
export async function searchSkills(query: string, options: { page?: number; limit?: number; sortBy?: 'stars' | 'recent' } = {}, signal?: AbortSignal): Promise<{ skills: SkillInfo[]; total: number }> {
  const params = new URLSearchParams({ q: query });
  params.set('page', String(options.page ?? 1));
  params.set('limit', String(Math.max(1, Math.min(50, options.limit ?? 20))));
  params.set('sortBy', options.sortBy ?? 'stars');
  const payload = await readJson(`${SKILLSMP_API}/skills/search?${params.toString()}`, signal) as {
    data?: { skills?: Array<Partial<SkillInfo> & { contentLanguage?: string }>; pagination?: { total?: number } };
  };
...
async function fetchRepoTree(owner: string, repo: string, branch: string, signal?: AbortSignal): Promise<TreeEntry[]> {
  const candidates = [branch, 'main', 'master'].filter((item, index, all) => all.indexOf(item) === index).slice(0, 3);
  let lastError: unknown;
  for (const candidate of candidates) {
    try {
      const payload = await readJson(`https://api.github.com/repos/${owner}/${repo}/git/trees/${candidate}?recursive=1`, signal) as { tree?: unknown[] };
      const entries = Array.isArray(payload.tree) ? payload.tree.slice(0, MAX_TREE_ENTRIES) : [];
      return entries.map(entry => {
        const record = entry as Partial<TreeEntry>;
        return { path: String(record.path ?? ''), type: record.type ?? 'blob', size: typeof record.size === 'number' ? record.size : undefined };
      });
    } catch (error) {
      lastError = error;
    }
  }
  throw lastError ?? new Error(`Could not fetch the repository tree for ${owner}/${repo}.`);
}

export async function listRepositorySkills(owner: string, repo: string, branch: string, signal?: AbortSignal): Promise<RepositorySkill[]> {
...
async function readRawFile(owner: string, repo: string, branch: string, filePath: string, signal?: AbortSignal): Promise<string> {
...
export async function readSkillMarkdown(owner: string, repo: string, branch: string, folderPath: string, signal?: AbortSignal): Promise<{ content: string; path: string }> {
  const base = folderPath.replace(/\/+$/, '');
  let lastError: unknown;
  for (const fileName of SKILL_FILE_NAMES) {
    const filePath = base ? `${base}/${fileName}` : fileName;
    try {
      return { content: await readRawFile(owner, repo, branch, filePath, signal), path: filePath };
    } catch (error) {
      lastError = error;
    }
  }
  throw lastError ?? new Error(`No ${SKILL_FILE_NAMES.join(' or ')} found in ${owner}/${repo}/${base || '.'}.`);
}
...
export function parseSkillFrontmatter(markdown: string): { name?: string; description?: string } {
  ...
  if (nameMatch?.[1]) result.name = nameMatch[1].trim().replace(/^['"]|['"]$/g, '');
...
export async function listInstalledSkills(skillsDir: vscode.Uri): Promise<InstalledSkill[]> {
  ...
  try {
    ...
    for (const [folder, type] of folders) {
      ...
      for (const fileName of SKILL_FILE_NAMES) {
        ...
        try {
          ...
          if ((stat.type & vscode.FileType.File) !== 0) {
            ...
            content = new TextDecoder().decode(await vscode.workspace.fs.readFile(candidate));
            break;
          }
        } catch {}
      }
      ...
      result.push({ name: meta.name ?? folder, description: meta.description ?? '', folder, skillMdPath });
    }
  } catch {}
...
export async function installSkillFromRepository(skillsDir: vscode.Uri, spec: { owner: string; repo: string; branch: string; folderPath: string; installName: string }, signal?: AbortSignal, onProgress?: (done: number, total: number) => void): Promise<InstallResult> {
  ...
  const files = entries.filter(entry => entry.type === 'blob' && (base ? entry.path.startsWith(prefix) : !entry.path.includes('/')))
    .slice(0, MAX_DOWNLOAD_FILES);
  if (!files.length) throw new Error(`No files found for skill '${spec.installName}' at ${spec.owner}/${spec.repo}/${base || '.'}.`);
  let totalBytes = 0;
  for (const entry of files) {
    totalBytes += entry.size ?? 0;
    if (totalBytes > MAX_DOWNLOAD_BYTES) throw new Error(`Skill '${spec.installName}' is too large to install (over ${Math.round(MAX_DOWNLOAD_BYTES / 1024 / 1024)} MB).`);
  }
  ...
  for (let index = 0; index < files.length; index++) {
    ...
    const content = await readRawFile(spec.owner, spec.repo, spec.branch, entry.path, signal);
  ...
  return { name: installName, files: files.length, bytes: writtenBytes, skillMdPath };
}

export async function uninstallSkill(skillsDir: vscode.Uri, folder: string): Promise<void> {
  ...
  if (targetPath !== skillsPath && !targetPath.startsWith(skillsPath + path.sep)) throw new Error('Skill path escapes the skills directory.');
  try {
    const stat = await vscode.workspace.fs.stat(target);
    if ((stat.type & vscode.FileType.Directory) === 0) throw new Error(`'${safeName}' is not a skill folder.`);
  } catch (error) {
    if (error instanceof vscode.FileSystemError && error.code === 'FileNotFound') throw new Error(`Skill '${safeName}' is not installed.`);
    throw error;
  }
  await vscode.workspace.fs.delete(target, { recursive: true });
}

export function resolveInstallPath(source: string, skill: string, branch: string): { owner: string; repo: string; branch: string; folderPath: string | undefined; hintedName: string | undefined } {
  const reference = parsedGithubReference(source);
  if (!reference) throw new Error(`Invalid GitHub reference '${source}'. Use 'owner/repo' or a github.com URL.`);
  return {
    owner: reference.owner,
    repo: reference.repo,
    branch: branch || reference.branch || 'main',
    folderPath: reference.path || undefined,
    hintedName: skill || (reference.path ? sanitizeSkillName(reference.path.split('/').pop() ?? 'skill') : undefined),
  };
}
