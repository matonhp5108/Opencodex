...
const NON_TEXT =
  /(?:^|[^a-z0-9])(?:vision|image|audio|speech|voice|tts|stt|asr|whisper|embed(?:dings?)?|rerank|moderation|transcri|imagen|dall-?e|flux|sora|veo|midjourney|stable-diffusion|4v|vl)(?:[^a-z0-9]|$)/i;
...
function modelEntries(body: unknown): ModelEntry[] {
  ...
  for (const item of record.data ?? []) {
    ...
    const id =
      typeof item.id === "string" ? item.id.replace(/^models\//, "") : "";
...
const GEMINI_FREE = (body: unknown): string[] =>
  modelIds(body)
    .filter((id) => /^(gemini-2\.[05]-|gemma-)/.test(id))
    .sort();
...
export function isCompatibleModel(
  provider: Provider,
  id: string,
  entry?: ModelEntry,
  ...
  if (provider.id === "openrouter" && entry?.raw) {
    ...
    if (Array.isArray(parameters) && !parameters.includes("tools"))
      return false;
    const outputs = (
      raw.architecture as { output_modalities?: unknown } | undefined
    )?.output_modalities;
    if (Array.isArray(outputs) && !outputs.includes("text")) return false;
  }
  if (provider.id === "ollama" && capabilities) {
    const caps =
      capabilities.get(id) ?? capabilities.get(id.replace(/:latest$/, ""));
    ...
    for (const model of models) {
      ...
      for (const key of [model.name, model.model]) {
        if (typeof key === "string" && key)
          map.set(key.replace(/:latest$/, ""), caps);
      }
    }
...
export async function fetchProviderModels(
  ...
  if (provider.needsApiKey) {
    if (!apiKey) {
      const env = provider.apiKeyEnvVar
        ? ` or set the ${provider.apiKeyEnvVar} environment variable`
        : "";
      throw new Error(
        `${provider.name} requires an API key. Open Settings and paste your key${env}.`,
      );
    }
  ...
  if (
    response.status === 401 ||
    response.status === 403 ||
    (response.status === 400 && keyHint)
  ) {
    throw new Error(
      `${provider.name} rejected the API key (HTTP ${response.status}). Check the key in Settings.`,
    );
  }
  if (!response.ok) {
    const detail = bodyText.trim().slice(0, 200);
    throw new Error(
      `${provider.name} returned HTTP ${response.status} while listing models${detail ? `: ${detail}` : "."}`,
    );
  }
  let body: unknown;
  try {
    ...
    throw new Error(
      `${provider.name} returned an unreadable response while listing models.`,
    );
  }
  ...
  const capabilities =
    provider.id === "ollama" && provider.isLocal
      ? await fetchOllamaCapabilities(baseURL)
      : undefined;
  const entryById = new Map(entries.map((entry) => [entry.id, entry]));
  models = models.filter((id) =>
    isCompatibleModel(provider, id, entryById.get(id), capabilities),
  );
